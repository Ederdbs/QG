...
Rscript -e 'devtools::test()'       # 184 checks (~1 min) — run after any change to R/
...
`test-pipeline.R`, `test-stage-metrics.R`, `test-f2size.R`). `tests/testthat/setup.R` builds
...
| `inst/scripts/` | `run_all.R`, `run_benchmark.R` — the CLI entry points at production scale |
...
A Quarto book: 12 chapters in 5 parts, plus 4 appendices. `book/_quarto.yml` lists them.
...
- **The two scripts in `inst/scripts/` are not dead weight.** They run at the *production*
  ...
  are ever called. Delete them and those three exports become unexercised.
